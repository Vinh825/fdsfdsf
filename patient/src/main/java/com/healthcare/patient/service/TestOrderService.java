...
public class TestOrderService {
    ...
    public ApiResponse<TestOrderResponseDTO> markCompleted(Long id, String runBy) {
        var order = testOrderRepository.findById(id)
                .filter(o -> !Boolean.TRUE.equals(o.getIsDeleted()))
                .orElseThrow(() -> new EntityNotFoundException("Test order not found"));
        ...
        audit.log(
                ...
                "COMPLETE",
                runBy,
                "status: " + oldStatus + " -> " + saved.getStatus() + ", runDate=" + saved.getRunDate()
        );

        return ApiResponse.success(toResponse(saved), "Marked order as COMPLETED");
    }

    public ApiResponse<Void> delete(Long id, String deletedBy) {
        var order = testOrderRepository.findById(id)
                .filter(o -> !Boolean.TRUE.equals(o.getIsDeleted()))
                .orElseThrow(() -> new EntityNotFoundException("Test order not found"));
        ...
        return ApiResponse.success(null, "Deleted test order by " + deletedBy);
    }
    ...
    public ApiResponse<TestOrderItemResponse> addItem(Long orderId, CreateTestOrderItemRequest dto) {
        ...
        audit.log(
                order.getId(),
                "ADD_ITEM",
                null, // tuỳ bạn gắn actor từ security context
                "itemId=" + saved.getId()
    ...
    public ApiResponse<TestOrderItemResponse> updateItem(Long orderId, Long itemId, UpdateTestOrderItemRequest dto) {
        var order = testOrderRepository.findById(orderId)
                .filter(o -> !Boolean.TRUE.equals(o.getIsDeleted()))
                .orElseThrow(() -> new EntityNotFoundException("Test order not found"));
        ...
        audit.log(
                ...
                null,
                "itemId=" + saved.getId()
                        + (dto.getStatus() != null ? (", status: " + beforeStatus + " -> " + saved.getStatus()) : "")
        );
